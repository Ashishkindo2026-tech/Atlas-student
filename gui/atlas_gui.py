...
class AtlasGUI(ctk.CTk):
    ...
    def _build_sidebar(self):
        ...
        ctk.CTkLabel(side, text="✦ ATLAS", text_color=GOLD_SOFT,
                     font=("Georgia", 25, "bold")).pack(anchor="w", padx=24, pady=(30, 0))
        ctk.CTkLabel(side, text="STUDENT", text_color=MUTED,
                     font=("Segoe UI", 9, "bold")).pack(anchor="w", padx=27, pady=(0, 24))
        items = [("⌂  Home", self.show_home), ("💬  Chat", self.show_chat),
        ...
        for text, cmd in items:
            ctk.CTkButton(side, text=text, command=cmd, anchor="w", fg_color="transparent",
                          hover_color="#3A2B21", text_color=CREAM, font=("Segoe UI", 11),
                          height=40).pack(fill="x", padx=8, pady=2)
        ctk.CTkButton(side, text="⚙  Settings", anchor="w", fg_color="transparent",
                      hover_color="#3A2B21", text_color=MUTED, height=40).pack(side="bottom", fill="x", padx=8, pady=20)

    def clear(self):
        for w in self.main.winfo_children(): w.destroy()

    def heading(self, title, subtitle=None):
        ctk.CTkLabel(self.main, text=title, text_color=CREAM,
                     font=("Georgia", 29, "bold")).pack(anchor="w", padx=42, pady=(34, 0))
        if subtitle:
            ctk.CTkLabel(self.main, text=subtitle, text_color=MUTED,
                         font=("Segoe UI", 11)).pack(anchor="w", padx=44, pady=(2, 22))

    def card(self, title, body="", parent=None):
        p = parent or self.main
        frame = ctk.CTkFrame(p, fg_color=CARD, corner_radius=14)
        ctk.CTkLabel(frame, text=title, text_color=GOLD_SOFT,
                     font=("Segoe UI", 12, "bold")).pack(anchor="w", padx=20, pady=(16, 4))
        if body:
            ctk.CTkLabel(frame, text=body, text_color=CREAM, justify="left",
                         wraplength=500, font=("Segoe UI", 10)).pack(anchor="w", padx=20, pady=(0, 16))
        return frame

    def show_home(self):
        self.clear(); self.heading("Good evening, Ashish.", "What are we learning today?")
        hero = ctk.CTkFrame(self.main, fg_color=PANEL, corner_radius=18)
        hero.pack(fill="x", padx=42)
        ctk.CTkLabel(hero, text="✦", text_color=GOLD, font=("Georgia", 34)).pack(anchor="w", padx=28, pady=(22, 0))
        ctk.CTkLabel(hero, text="Your learning, memories and goals — together.", text_color=CREAM,
                     font=("Georgia", 19, "bold")).pack(anchor="w", padx=28, pady=5)
        ctk.CTkLabel(hero, text="Atlas Student is ready.", text_color=MUTED,
                     font=("Segoe UI", 10)).pack(anchor="w", padx=28)
        ctk.CTkButton(hero, text="Start a conversation  →", command=self.show_chat,
                      fg_color=GOLD, hover_color=GOLD_SOFT, text_color="#1B130D",
                      font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=28, pady=20)
        row = ctk.CTkFrame(self.main, fg_color="transparent")
        row.pack(fill="x", padx=42, pady=20)
        for title, body in [("📚 Continue learning", "Class 11 • Physics"),
                            ("🎯 Goals", "Keep building Atlas Student"),
                            ("📈 Progress", "Your learning profile is growing")]:
            self.card(title, body, row).pack(side="left", fill="both", expand=True, padx=(0, 12))

    def show_chat(self):
        self.clear(); self.heading("Atlas", "Ask, learn, plan.")
        chat = ctk.CTkTextbox(self.main, fg_color=PANEL, text_color=CREAM,
                              font=("Segoe UI", 11), corner_radius=14)
        ...
        entry = ctk.CTkEntry(row, placeholder_text="Ask Atlas anything...", height=42,
                             fg_color="#241B16", border_width=0, text_color=CREAM)
        ...
        ctk.CTkButton(row, text="Send  →", command=send, width=100,
                      fg_color=GOLD, hover_color=GOLD_SOFT, text_color="#1B130D").pack()

    def show_library(self):
        self.clear(); self.heading("My Library", "CBSE core • Classes 9–12")
        ctk.CTkButton(self.main, text="＋ Add Book PDF", command=self.add_book,
                      fg_color=GOLD, hover_color=GOLD_SOFT, text_color="#1B130D",
                      font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=42, pady=(0, 18))
