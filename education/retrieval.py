...
def retrieve(query: str, limit: int = 5) -> List[Dict]:
    return [
        {"text": r.chunk.text, "page": r.chunk.page, "chapter": r.chunk.chapter,
         "section": r.chunk.section, "source": r.chunk.source, "score": r.score}
        for r in load_index().search(query, limit=limit)
    ]
...
def build_context(query: str, limit: int = 5) -> str:
    ...
    for i, item in enumerate(results, 1):
        ...
        blocks.append(f"[Education source {i} | {item['source']} | {location}]\n{item['text']}")
    return "\n\n".join(blocks)
